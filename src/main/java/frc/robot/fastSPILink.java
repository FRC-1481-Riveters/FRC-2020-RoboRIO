 ...
 *         end license header
 *
 *         Arduino ICSP fastSPI link class
 */

public class fastSPILink implements Link {
    private int PIXY_SPI_CLOCKRATE = 2000;

    private SPI spi = null;

    public fastSPILink(int speedInHz) {

        PIXY_SPI_CLOCKRATE = speedInHz;

        if (spi != null) {
            spi.setClockRate(speedInHz);
        }
    }
